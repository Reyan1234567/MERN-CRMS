...
export default function Vehicle() {
  ...
  async function onSubmit(e: any) {
    ...
    try {
      let response;
      if (isNew) {
        response = await fetch("http://localhost:5050/vehicle", {
          method: "POST",
          headers: {
        ...
        response = await fetch(`http://localhost:5050/vehicle/${params.id}`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  return (
    <>
      <h3 className="text-lg font-semibold p-4">Create/Update Vehicle</h3>
      <form onSubmit={onSubmit} className="border rounded-lg overflow-hidden p-4">
        ...
        <input
          type="submit"
          value={isNew ? "Save Vehicle Record" : "Update Vehicle Record"}
...
//       const vehicle = await response.json();
//       if (!vehicle) {
//         console.warn(`Vehilce with id ${id} not found`);
//         navigate("/");
//         return;
//       }
//       setForm(vehicle);
